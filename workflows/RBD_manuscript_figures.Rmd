...
plot_data <- setNames(vector(mode = "list", length = 2),
...
plot_data[[1]] <- 
  ...
  dplyr::inner_join(results$robust %>% 
...
plot_tibble <- 
  tibble(dataset = c("eQTLGen", "psychencode"),
...
for(i in 1:nrow(plot_tibble)){
  ...
  fig_list[[i]] <- 
    plot_coloc_hits(coloc_hits = results$robust %>%
                      dplyr::filter(dataset == plot_tibble$dataset[i], PP.H4.abf >= 0.75) %>% 
                      dplyr::arrange(hgnc_symbol) %>% 
                      .[["gene_2"]],
                    eQTL_GWAS_to_plot = plot_data[[i]], 
                    facet_labels = c(pvalue_gwas = plot_tibble$GWAS_label[i], pvalue_eqtl = plot_tibble$dataset_label[i]),
...
supp_fig <- 
  plot_coloc_hits(coloc_hits = results$robust %>%
                    dplyr::filter(dataset == "psychencode", hgnc_symbol == "DLG5") %>% 
                    dplyr::arrange(hgnc_symbol) %>% 
                    .[["gene_2"]],
                  eQTL_GWAS_to_plot = plot_data$psychencode, 
                  facet_labels = c(pvalue_gwas = plot_tibble$GWAS_label[2], pvalue_eqtl = plot_tibble$dataset_label[2]),
                  theme_base_size = theme_base_size) 
...
**Regional association plot for eQTL and RBD GWAS colocalisation in the region surrounding *DLG5*.** Regional association plots for eQTL (upper pane) and RBD GWAS association signals (lower pane) in the regions surrounding *DLG5* (PPH4 = `r results$robust %>% dplyr::filter(dataset == "psychencode", hgnc_symbol == "DLG5") %>% .[["PP.H4.abf"]] %>% round(digits = 2)`). eQTLs are derived from PsychENCODE's analysis of adult brain tissue from 1387 individuals. The x-axis denotes chromosomal position in hg19, and the y-axis indicates association p-values on a -log~10~ scale.

## Supplementary table
```{r}

supp_table <- setNames(vector(mode = "list", length = 2), 
...
supp_table[[2]] <- 
  ...
  dplyr::mutate(hgnc_symbol = case_when(gene_2 == "ENSG00000247775" ~ "SNCA-AS1",
                                        TRUE ~ hgnc_symbol)) %>% 
  dplyr::select(dataset, ensembl_id = gene_2, hgnc_symbol, 
                PP_H0 = PP.H0.abf, PP_H1 = PP.H1.abf, PP_H2 = PP.H2.abf, PP_H3 = PP.H3.abf, PP_H4 = PP.H4.abf,
                sum_PPH3_PPH4, ratio_PPH4_PPH3) %>% 
  dplyr::arrange(dataset, ensembl_id)

supp_table[[1]] <- 
  tibble(`Column name` = colnames(supp_table[[2]]),
         Description = c("eQTL dataset name",
                         "Gene ensembl ID",
                         "Gene HGNC symbol",
...
ggsave("figure_coloc.png", fig, device = "png", 
...
ggsave("figure_coloc.pdf", fig, device = "pdf", 
       path = path_for_figures,
       width = 180,  
       height = 120,
       units = "mm",
...
ggsave("supp_figure_coloc.png", supp_fig, device = "png", 
       path = path_for_figures,
       width = 90,  
       height = 120,
       units = "mm",
...
plot_data <- 
  ...
  dplyr::inner_join(results$robust %>% 
                      dplyr::filter(PP.H4.abf >= pph4_decision | hgnc_symbol == "DLG5" & dataset == "psychencode") %>% 
                      dplyr::mutate(hgnc_symbol = case_when(gene_2 == "ENSG00000247775" ~ "SNCA-AS1",
                                                            TRUE ~ hgnc_symbol)) %>% 
                      dplyr::distinct(gene_2, dataset, hgnc_symbol), by = c("gene" = "gene_2", "dataset")) %>% 
...
for(i in 1:nrow(plot_data)){
  ...
  png(file = str_c(path_for_figures, "supp_figure_", plot_data$dataset[i], "_", plot_data$hgnc_symbol[i], ".png"),
      width = 180, 
      height = 120,
      units = "mm", 
      res = 300)
...
#### DLG5
**Sensitivity analysis of *DLG5* colocalisation.** Sensitivity analysis of colocalisation between PscyhENCODE-derived eQTLs regulating *DLG5* expression and RBD GWAS signals. Plot of prior (left) and posterior (right) probabilities for H0-H4 across varying p~12~ priors. Dashed vertical line indicates the value of p~12~ used in the initial analysis (p~12~ = 5 x $10^{-6}$). The green region in these plots show the region for which PPH4 $\geq$ `r pph4_decision` would still be supported. 
...
plot_data <- 
  ...
  dplyr::rename(pvalue_gwas = p.value_1,
                pvalue_eqtl = p.value_2,
                beta_gwas = beta_1,
                beta_eqtl = beta_2) %>% 
  dplyr::mutate(genome_wide_signif = 
                  case_when(pvalue_gwas < 5e-8 | pvalue_eqtl < 5e-8 ~ "TRUE",
                                               TRUE ~ "FALSE") %>% 
...
fig <- 
  ...
  scale_colour_manual(values = c("#EE442F", "#888888"), 
...
ggsave("supp_figure_betas.png", fig, device = "png", 
       path = path_for_figures,
       width = 140,  
       height = 120,
       units = "mm",
...
ggsave("supp_figure_betas.pdf", fig, device = "pdf", 
       path = path_for_figures,
       width = 140,  
       height = 120,
       units = "mm",
...
fig_list <- setNames(vector(mode = "list", length = 2),
                     c("gtex", "aibs"))

fig_list$gtex <- 
  plot_gtex_specificity(specificity = plot_data$gtex, 
                        theme_base_size = theme_base_size_small)
fig_list$aibs <- 
  plot_aibs_specificity(specificity = plot_data$aibs, 
                        theme_base_size = theme_base_size_small)
...
supp_table[[1]] <- 
  tibble(`Column name` = colnames(supp_table[[2]]),
         Description = c("Dataset from which specificity values were derived",
                         "Gene HGNC symbol",
...
ggsave("figure_specificity.png", fig, device = "png", 
       path = path_for_figures,
       width = 180,  
       height = 200,
       units = "mm",
...
ggsave("figure_specificity.pdf", fig, device = "pdf", 
       path = path_for_figures,
       width = 180,  
       height = 200,
       units = "mm",
